...
(function () {
  ...
  function setLang(lang) {
    ...
    renderProjects(lang);
    renderTimeline(lang);
    // CV button label + live refresh if the CV popup is already open
    ...
    if (cvModal && cvModal.classList.contains("is-open")) setCvSource(lang);
  }
  ...
  function closeModalEl(el) {
    ...
    if (el.id === "cvModal") { var c = document.getElementById("cvPages"); if (c) c.innerHTML = ""; }
  }
  ...
  function setCvSource(lang) {
    var file = P.cvFiles[lang];
    var pages = (P.cvPages && P.cvPages[lang]) || [];
    var alt = lang === "de" ? "Lebenslauf" : "Curriculum Vitae";
    var cont = document.getElementById("cvPages");
    var openTab = document.getElementById("cvOpenTab");
    var title = document.getElementById("cvModalTitle");
    if (cont) cont.innerHTML = pages.map(function (src) {
      return '<img loading="lazy" src="' + src + '" alt="' + alt + '">';
    }).join("");
    if (openTab) openTab.setAttribute("href", file);
    if (title) title.textContent = alt;
  }
  function openCvModal() {
    setCvSource(CURRENT_LANG);
    openModalEl(document.getElementById("cvModal"));
  }
  ...
  function initModal() {
    ...
    document.querySelectorAll(".modal").forEach(function (m) {
      m.addEventListener("click", function (e) { if (e.target.hasAttribute("data-close")) closeModalEl(m); });
    });
